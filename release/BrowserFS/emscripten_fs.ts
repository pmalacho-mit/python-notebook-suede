    ...
    try {
      if (FS.isFile(stream.node.mode)) {
        stream.nfd = this.nodefs.openSync(
          path,
          this.efs.flagsToPermissionString(stream.flags),
        );
      }
    } catch (e: any) {
      if (!e.code) {
...
export default class BFSEmscriptenFS implements EmscriptenFS {
  ...
  private nodefs: FSLike;
  constructor(
    _FS = (<any>globalThis)["FS"],
    _PATH = (<any>globalThis)["PATH"],
    _ERRNO_CODES = (<any>globalThis)["ERRNO_CODES"],
    nodefs: FSLike,
  ) {
    this.nodefs = nodefs;
    this.FS = _FS;
